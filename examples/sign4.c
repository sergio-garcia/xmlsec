 ...
 * 
 * Usage: 
 *      sign4 <xml-doc> <pem-key> 
 *
 * Example:
 *      ./sign4 sign4-doc.xml rsakey.pem rsacert.pem > sign4-res.xml
 *
...
sign_file(const char* xml_file, const char* key_file, const char* cert_file) {
    ...
    if(x509DataNode->children == NULL) {
        printf("children 1\n");
    }
    
    if(xmlSecTmplX509DataAddSKI(x509DataNode) == NULL) {
        fprintf(stderr, "Error: failed to add X509SKI node\n");
        goto done;              
    }
    printf("me\n");
    if(x509DataNode->children == NULL) {
    ...
    if(x509IssuerSerialNode == NULL) {
        fprintf(stderr, "Error: failed to add X509IssuerSerial node\n");
        goto done;              
    }

    if(xmlSecTmplX509IssuerSerialAddIssuerName(x509IssuerSerialNode, "Test Issuer") == NULL) {
        fprintf(stderr, "Error: failed to add X509IssuerName node\n");
        goto done;              
    }

    if(xmlSecTmplX509IssuerSerialAddSerialNumber(x509IssuerSerialNode, "1") == NULL) {
        fprintf(stderr, "Error: failed to add X509SerialNumber node\n");
        goto done;              
    }
